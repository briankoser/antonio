...
function antonio() {
    ...
    $.getJSON('http://bgg-users-want.azurewebsites.net/api/game/' + gameID, getUsersWanting);
}
...
function getUsersWanting(data) {
    ...
    $.each(data.UsersWant, getUserWants);
}

function getUserWants(index, item) {
    ...
    if (userName === 'klurejr' || userName === 'Base the Bass' || userName === 'tavlas') {
        ...
        $.getJSON(url, storeGamesForTrade);
    }
...
function storeGamesForTrade(data) {
